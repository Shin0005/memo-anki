...
※ドキュメントはgitの履歴が汚くなるためNotionで管理している
...
バックエンド重視という考えから、フロントは副作用ロジックと分岐の多いフォームに絞って自動テストを書く方針とした。表示専用コンポーネントや純粋な関数は手動確認で十分と判断して省いた。

| 対象                                                                                                                                                                 | 何をテストするか                                        | 採用理由                                                                                     |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `lib/api/client.ts`                                                                                                                                                  | 401 → Refresh Token で AT 更新 → 元リクエスト再試行     | 全 API 呼び出しを通る共通経路。手動では AT 期限切れの再現コストが高い                        |
| `features/card/hooks/useCardMutations.ts`                                                                                                                            | HTTP ステータス別のトースト分岐とキャッシュ無効化       | useDeck/useAuth と同構造のため代表1つだけ書く                                                |
| `features/card/components/CardCreateModal.tsx`                                                                                                                       | isQuiz トグル時のフィールド切替と submit バリデーション | useEffect + resetField + watch の組み合わせはバグが入りやすい                                |
| `features/card/utils/cardView.ts`                                                                                                                                    | NOTE/QUIZ の表示文字列組み立てと日付整形                | 純粋関数でコスパが高い                                                                       |
| `features/review/lib/shouldFetch.ts` `features/review/lib/mergeQueue.test.ts` `features/review/hooks/useReviewMutation.ts` `features/review/hooks/useReviewQueue.ts` | 復習ロジックの中核                                      | 本プロジェクトにおける最重要ロジックでありこの機能が保証されていないとアプリの利用価値がない |

省略した対象と理由:

- `useDeckMutations` / `useAuthMutations`: useCardMutations と構造同一
- `RegisterForm` のパスワード一致チェック: 1行の `if`、手動確認で十分
- `middleware.ts`: 分岐は Cookie 有無の1つのみ
- 表示専用コンポーネント (DeckGrid 等): クラス文字列の差し替えのみ

BE/FE で Vitest を統一している。網羅性ではなく優先順位を意識した。
...
| Phase3   | Deck/Card CRUD（NOTE・QUIZはCardのtype）  | 完了 |
| Phase4   | 復習ロジック（キュー管理・ステート遷移）  | 完了 |
