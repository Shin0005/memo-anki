...
| **updateCard** | 正常系: 変換確認       | Requestの `cardId` (string) が `BigInt` に変換され Service へ渡されること      | 型変換               |
|                | バリデーション         | `cardId` が非数字や20桁超の場合、400エラーとなること                           | BigInt境界値         |
| **deleteCard** | 正常系: 完了レスポンス | 成功時に Status 204 (No Content) を返すこと                                    | REST準拠             |
